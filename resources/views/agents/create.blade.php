<x-layout>
    ...
    <div class="border border-gray-600 p-4 rounded-lg my-4 bg-gray-800 w-1/2 m-auto">
        <form action="{{ route('tugas.agents.store') }}" method="post" enctype="multipart/form-data">
            @csrf

            <h3 class="text-2xl font-semibold mb-4">Add New Agents</h3>

            <div class="mb-3">
                <label for="name" class="block mb-2">Name</label>
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Name" class="w-full block bg-gray-700 border border-gray-600 text-sm rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-400 p-2 pl-3 focus:outline-0 @error('name') border-red-600 focus:ring-red-600 focus:border-red-600 @enderror">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="date" name="release_date" value="{{ old('release_date') }}" id="release_date" class="w-full block bg-gray-700 border border-gray-600 text-sm rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-400 p-2 pl-3 focus:outline-0 @error('release_date') border-red-600 focus:ring-red-600 focus:border-red-600 @enderror">
                @error('release_date')
                    <p class="text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
